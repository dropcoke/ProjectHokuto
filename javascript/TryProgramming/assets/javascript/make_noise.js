...
const playD2 = () => {
    ...
    playMySynth.markNote(3, 1, noteLength);
}
const playE2 = () => {
    startSound(sound, 17, noteLength);
    playMySynth.markNote(3, 2, noteLength);
}
const playF2 = () => {
    startSound(sound, 18, noteLength);
    playMySynth.markNote(3, 3, noteLength);
}
const playG2 = () => {
    startSound(sound, 19, noteLength);
    playMySynth.markNote(3, 4, noteLength);
}
const playA2 = () => {
    startSound(sound, 20, noteLength);
    playMySynth.markNote(3, 5, noteLength);
}
const playB2 = () => {
    startSound(sound, 21, noteLength);
    playMySynth.markNote(3, 6, noteLength);
}
const playC3 = () => {
...
const playD3 = () => {
    ...
    playMySynth.markNote(6, 1, noteLength);
}
const playE3 = () => {
    startSound(sound, 24, noteLength);
    playMySynth.markNote(6, 2, noteLength);
}
const playF3 = () => {
    startSound(sound, 25, noteLength);
    playMySynth.markNote(6, 3, noteLength);
}
const playG3 = () => {
    startSound(sound, 26, noteLength);
    playMySynth.markNote(6, 4, noteLength);
}
const playA3 = () => {
    startSound(sound, 27, noteLength);
    playMySynth.markNote(6, 5, noteLength);
}
const playB3 = () => {
    startSound(sound, 28, noteLength);
    playMySynth.markNote(6, 6, noteLength);
}

// マウスクリック
const onC1 = () => {
    playMySynth.startSound(sound, 8);
    playMySynth.markNote(0, 0);
}
const onD1 = () => {
    playMySynth.startSound(sound, 9);
    playMySynth.markNote(0, 1);
}
const onE1 = () => {
    playMySynth.startSound(sound, 10);
    playMySynth.markNote(0, 2);
}
const onF1 = () => {
    playMySynth.startSound(sound, 11);
    playMySynth.markNote(0, 3);
}
const onG1 = () => {
    playMySynth.startSound(sound, 12);
    playMySynth.markNote(0, 4);
}
const onA1 = () => {
    playMySynth.startSound(sound, 13);
    playMySynth.markNote(0, 5);
}
const onB1 = () => {
    playMySynth.startSound(sound, 14);
    playMySynth.markNote(0, 6);
}
const onC2 = () => {
    playMySynth.startSound(sound, 15);
    playMySynth.markNote(3, 0);
}
const onD2 = () => {
    playMySynth.startSound(sound, 16);
    playMySynth.markNote(3, 1);
}
const onE2 = () => {
    playMySynth.startSound(sound, 17);
    playMySynth.markNote(3, 2);
}
const onF2 = () => {
    playMySynth.startSound(sound, 18);
    playMySynth.markNote(3, 3);
}
const onG2 = () => {
    playMySynth.startSound(sound, 19);
    playMySynth.markNote(3, 4);
}
const onA2 = () => {
    playMySynth.startSound(sound, 20);
    playMySynth.markNote(3, 5);
}
const onB2 = () => {
    playMySynth.startSound(sound, 21);
    playMySynth.markNote(3, 6);
}
const onC3 = () => {
...
const onD3 = () => {
    ...
    playMySynth.markNote(6, 1);
}
const onE3 = () => {
    playMySynth.startSound(sound, 24);
    playMySynth.markNote(6, 2);
}
const onF3 = () => {
    playMySynth.startSound(sound, 25);
    playMySynth.markNote(6, 3);
}
const onG3 = () => {
    playMySynth.startSound(sound, 26);
    playMySynth.markNote(6, 4);
}
const onA3 = () => {
    playMySynth.startSound(sound, 27);
    playMySynth.markNote(6, 5);
}
const onB3 = () => {
    playMySynth.startSound(sound, 28);
    playMySynth.markNote(6, 6);
}
...
const PlayMySynth = {
    ...
    markNote(y, x, length) {
        ...
        this.keyOn = true;
        this.getCellElement(y, x).style.background = keyColor[x];
        this.currentNotCell = {y: y, x: x};
        // setTimeout(() => {
        //     this.getCellElement(this.currentNotCell.y, this.currentNotCell.x).style.background = 'white';
        ...
        for (let i = 0; i < y; i++) {
            this.cells[i] = []
                for (let j = 0; j < x; j++) {
                    ...
                    this.cells[i][j].addEventListener('mousedown', (e) => {
                        let target = e.target;
                        if(!e.target.id) {
    ...
    playSound(oscillator, note = 0, length = 1) {
        const volume = 1;
        if (note <= 0 || note > this.keys.length){
            throw new Error('その音程はありません' + note);
        }
        if (oscillator <= 0 || oscillator > this.oscillatorTypes.length){
        ...
        if (this.playing) {
            ...
            this.synth.play(-10, this.notes[this.keys[note - 1]], this.oscillatorTypes[oscillator - 1]);
            setTimeout(() => {this.stopSound();}, length * 1000);
            this.currentLength = length * 1000;
            this.playing = true;
        }
    },
    startSound(oscillator, note = 0) {
        this.synth.play(-10, this.notes[this.keys[note - 1]], this.oscillatorTypes[oscillator - 1]);
        this.playing = true;
    },
    stopSound() {
